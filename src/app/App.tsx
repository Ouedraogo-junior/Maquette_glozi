...
import { IndividualSellerView } from "@/app/components/individual-seller-view";
import { BackOffice } from "@/app/components/back-office";
import { OnboardingFlow } from "@/app/components/auth-onboarding";
import { Toaster } from "sonner";

type Platform = "buyer" | "seller" | "individualSeller" | "admin" | "onboarding";
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-neutral-50 text-neutral-900 font-sans">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 md:left-auto md:translate-x-0 md:right-6 z-50 bg-white/80 backdrop-blur-md border border-neutral-200 rounded-full px-4 py-2 shadow-xl flex items-center gap-3">
        <button
        ...
        <button
          onClick={() => setPlatform("individualSeller")}
          className={`flex items-center gap-2 px-3 py-1.5 rounded-full transition-all ${platform === "individualSeller" ? "bg-purple-600 text-white shadow-md" : "hover:bg-neutral-100"}`}
        >
          <Package size={16} />
          <span className="text-xs font-medium">Vente Part.</span>
        </button>
        <button
      ...
      <main className="min-h-screen overflow-y-auto pb-32">
        ...
        <AnimatePresence mode="wait">
          ...
          {platform === "individualSeller" && (
            <motion.div
              key="individualSeller"
              initial={{ opacity: 0, scale: 0.98 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.98 }}
              className="h-full flex items-center justify-center p-4 md:p-8"
